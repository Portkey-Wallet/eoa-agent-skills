...
export function generateIronclawServerEntry(
  ...
  return {
    ...
    headers: {},
    enabled: true,
    description: 'Portkey EOA wallet MCP server for IronClaw',
  };
}
...
export function assertSafeIronclawSkillDir(skillsDir: string, skillDir: string): void {
  ...
  const expectedSkillDir = path.dirname(
    getIronclawSkillInstallPath(resolvedSkillsDir),
  );
  ...
  if (
    ...
    throw new Error(
      `Refusing to remove unexpected IronClaw skill path: ${resolvedSkillDir}`,
    );
  }
}
...
export function setupIronclaw(opts: {
  ...
  console.log(
    '\nRemember to replace <YOUR_PRIVATE_KEY> and <YOUR_WALLET_PASSWORD> in the IronClaw MCP config.',
  );
