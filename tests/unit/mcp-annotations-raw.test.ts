...
async function waitForResponse(
  ...
  while (Date.now() < deadline) {
    ...
    if (proc.exitCode !== null) {
      throw new Error(
        `MCP server exited before tools/list response. stderr: ${stderrState.value}`,
      );
    }
    await Bun.sleep(20);
  }
  ...
  throw new Error(
    `Timed out waiting for tools/list response. stderr: ${stderrState.value}`,
  );
}
...
describe('MCP raw annotation contract', () => {
  ...
  test('raw tools/list includes both camelCase and snake_case approval hints', async () => {
    ...
    const localWrite = tools.find((tool) => tool.name === 'portkey_create_wallet');
    const networkWrite = tools.find((tool) => tool.name === 'portkey_transfer');
    const readOnly = tools.find((tool) => tool.name === 'portkey_get_wallet_info');

    expect(localWrite?.annotations?.destructiveHint).toBe(true);
    expect(localWrite?.annotations?.destructive_hint).toBe(true);
    expect(localWrite?.annotations?.openWorldHint).toBeUndefined();
