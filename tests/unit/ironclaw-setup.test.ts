...
describe('IronClaw setup workflow', () => {
  ...
  beforeEach(() => {
    testRoot = join(tmpdir(), `portkey-ironclaw-${Date.now()}-${Math.random()}`);
  ...
  test('setup writes trusted skill and MCP config without creating installed_skills', () => {
    ...
    expect(readFileSync(installedSkillPath, 'utf8')).toContain('portkey-eoa-agent-skills');
  });
  ...
  test('re-running setup without force keeps a single MCP entry and skill file', () => {
    ...
    expect(
      config.servers.filter((server) => server.name === 'portkey-eoa-agent-skills'),
    ).toHaveLength(1);
  });
  ...
  test('defensive removal rejects unexpected skill paths', () => {
    const unexpectedSkillDir = join(testRoot, 'unexpected', 'portkey-eoa-agent-skills');

    expect(() =>
      assertSafeIronclawSkillDir(skillsDir, unexpectedSkillDir),
    ).toThrow('Refusing to remove unexpected IronClaw skill path');
  });
});
