...
describe('wallet context storage', () => {
  ...
  beforeEach(() => {
    tempDir = join(tmpdir(), `eoa-wallet-context-${Date.now()}-${Math.random()}`);
  ...
  test('setActiveWalletProfile creates directory and context file', () => {
    ...
    expect(readWalletContext()?.profiles.default?.address).toBe('ELF_test_AELF');
  });
  ...
  test('cleans stale lock and continues writing', () => {
    ...
    const context = setActiveWalletProfile(
      {
        walletType: 'EOA',
        source: 'eoa-local',
    ...
    expect(context.profiles.default?.address).toBe('ELF_stale_AELF');
